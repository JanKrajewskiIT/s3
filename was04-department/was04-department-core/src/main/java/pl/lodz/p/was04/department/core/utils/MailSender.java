...
public class MailSender {
    ...
    /**
     * Creates a pending invitation in the DB and sends an invitation mail to the given e-mail address. Invokes {@link PendingInvitationFacadeLocal#create(pl.lodz.p.was04.headoffice.entity.PendingInvitation)
     * }.
     *
     * @author Łukasz Gadomski
     * @param registrationURL the URL of registration page
     * @param email the e-mail address of the recipient
     * @param role the role that the new user is going to receive
     */
    public void sendInvitationMail(String registrationURL, String email, Role role) {
        try {
            PendingInvitation oldPendingIvInvitation = pendingInvitationDao.findByEmail(email);
            if (oldPendingIvInvitation != null) {
                pendingInvitationDao.delete(oldPendingIvInvitation);
            }

            String token = DigestUtils.sha256Hex(email + UUID.randomUUID().toString());
    ...
    public void sendPasswordChangeRequestMail(String passwordResetURL, User user) {
        try {
            PasswordChangeRequest oldPasswordChangeRequest = passwordChangeRequestDao.findByUser(user);
            if (oldPasswordChangeRequest != null) {
                passwordChangeRequestDao.delete(oldPasswordChangeRequest);
            }
            String email = user.getEmail();
            PasswordChangeRequest passwordChangeRequest = new PasswordChangeRequest();
            passwordChangeRequest.setId(DigestUtils.sha256Hex(user.getEmail() + UUID.randomUUID().toString()));
            ...
            message.setText(
                    ...
                    + passwordResetURL + "?id=" + passwordChangeRequest.getId() + " \n");
            Transport.send(message);
        } catch (MessagingException | UniqueConstraintViolationException ex) {
            Logger.getLogger(MailSender.class.getName()).log(Level.SEVERE, null, ex);
        }
